...
outer:
	for {
		// Try to serve high-priority request ASAP
		for {
			select {
			case takeHigh <- res: // High priority request took resource
				res = <-giveBack
				continue outer
			default:
			}
		...
		//  TODO: is there a better way of doing this? Since we didnt have a high priority request, we can serve low priority request.
		//  but if we get a high priority request while in this part of the code, we cannot prioritize it, afaik. So its just RNG right now.
		//  It works though so its fine by me :)
		select {
		case takeHigh <- res:
			res = <-giveBack
		case takeLow <- res:
